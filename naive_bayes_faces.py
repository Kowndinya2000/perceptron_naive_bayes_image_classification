...
if __name__ == "__main__":
    ...
    prob_table_face = [[[0 for _ in range(101)]
                        for _ in range(7)] for _ in range(7)]
    prob_table_not_face = [
        [[0 for _ in range(101)] for _ in range(7)] for _ in range(7)]
    num_faces = 0
    ...
    for ino in range(len(test_data)):
        ...
        for x in range(7):
            for y in range(7):
                factor = prob_table_face[x][y][features_test[ino][x][y]]/sum(
                    prob_table_face[x][y]) if prob_table_face[x][y][features_test[ino][x][y]] != 0 else 1/num_faces
                pface *= factor
                factor2 = prob_table_not_face[x][y][features_test[ino][x][y]]/sum(
                    prob_table_not_face[x][y]) if prob_table_not_face[x][y][features_test[ino][x][y]] != 0 else 1/(len(inos)-num_faces)
                pnotface *= factor2
    ...
    for ino in range(len(validation_data)):
        ...
        for x in range(7):
            for y in range(7):
                factor = prob_table_face[x][y][features_validation[ino][x][y]]/sum(
                    prob_table_face[x][y]) if prob_table_face[x][y][features_validation[ino][x][y]] != 0 else 1/num_faces
                pface *= factor
                factor2 = prob_table_not_face[x][y][features_validation[ino][x][y]]/sum(
                    prob_table_not_face[x][y]) if prob_table_not_face[x][y][features_validation[ino][x][y]] != 0 else 1/(len(inos)-num_faces)
                pnotface *= factor2
